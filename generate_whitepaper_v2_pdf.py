#!/usr/bin/env python3
"""Generate a vendor-neutral PDF from the Agentic Data Foundry whitepaper (V2)."""
...
OUTPUT_PDF = os.path.join(OUTPUT_DIR, "WHITEPAPER-V2.pdf")
...
def build_cover(elements, styles):
    cover_bg = Table(
        [
            ...
            [Spacer(1, 0.3 * inch)],
            [Paragraph("A Vendor-Neutral Whitepaper on AI-Native Data Engineering", styles['CoverSubtitle'])],
            [Spacer(1, 0.15 * inch)],
    ...
    elements.append(Paragraph(
        '<font color="#6C757D"><i>Cloud-Native  |  LLM-Powered  |  Declarative Materialization  |  CDC  |  Knowledge Graph</i></font>',
...
def build_abstract(elements, styles):
    ...
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        "Every major cloud data platform now offers built-in LLM services, declarative materialized views, and "
        "agentic AI capabilities \u2014 but these tools are only as effective as the data they consume. If your "
        "\"Back-of-House\" data supply chain is still a web of brittle, human-dependent pipes, your AI agents "
        "will inevitably fail.",
        styles['AbstractText']))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        "The <b>Agentic Data Foundry</b> is the missing architectural layer. It represents a fundamental shift from "
        "<b>Building</b> to <b>Describing</b>. By leveraging modern cloud-native capabilities \u2014 CDC connectors "
        "for ingestion, declarative materialized views for orchestration, and LLMs for reasoning \u2014 the pattern "
        "enables a system that autonomously discovers, transforms, and validates data. In this paradigm, the data "
        "engineer doesn't write code; they <b>curate intent</b>. This is the blueprint for an autonomous data lifecycle "
        "that moves at the speed of business thought, ensuring that the data platform becomes the central "
        "<b>System of Action</b> for the modern enterprise.",
        styles['AbstractText']))
...
def build_section1(elements, styles):
    ...
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        "We don't have a data problem; we have a <b>coordination problem</b>. Agentic AI platforms and the era of \"Agentic "
    ...
    elements.append(Paragraph(
        ...
        "Every major cloud data platform \u2014 Snowflake, Databricks, BigQuery, Fabric \u2014 has invested "
        "heavily in native LLM integration, declarative data pipelines, and agentic frameworks.",
        styles['BodyText2']))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        "The strategic direction across the industry is converging. Platform vendors are announcing autonomous AI "
        "capabilities designed to help business users \"simply ask for what they need\" and have AI \"securely "
        "complete multi-step tasks.\" The shift from systems of <i>insight</i> to systems of <i>action</i> is where "
        "measurable business value is ultimately realized. The Agentic Data Foundry embodies this same architectural "
        "principle \u2014 applied specifically to data engineering \u2014 where governed data, shared business definitions, "
        "and autonomous execution converge to replace manual pipeline construction with intent-driven automation.",
        styles['BodyText2']))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        "The Agentic Data Foundry synthesizes these trends into a deployable architecture that can autonomously discover "
        "tables, generate transformation logic, validate results, and learn from its own execution history \u2014 all "
        "guided by human-defined intent rather than human-written code.",
        styles['BodyText2']))
...
def build_section2(elements, styles):
    ...
    t = make_table(
        ...
        [
            ...
            ["Learnings", "Capture accumulated knowledge from past executions",
             "\"Tables with _UPDATED_AT require CDC deduplication via ROW_NUMBER().\""],
        ],
    ...
    elements.append(Paragraph(
        ...
        "automating schema inference, anomaly detection, and pipeline generation [4]. Capgemini's research on AI-driven "
        "data integration similarly concludes that the future lies in \"AI-powered orchestration of data flows rather than "
        "manual pipeline construction\" [5].",
        styles['BodyText2']))
    elements.append(Spacer(1, 10))
...
def build_section3(elements, styles):
    ...
    Schema-on-Read  Materialized Views  Aggregation Views
    ...
    journey = [
        ...
        ["2", "CDC Connector", "Row appears in landing schema with _updated_at CDC timestamp set"],
        ["3", "Bronze Capture", "Schema-on-read absorbs change into CUSTOMERS_RAW (semi-structured)"],
        ...
        ["9", "AI Consumption", "Business user asks the semantic layer: \"Which customers are at risk of churning?\""],
    ]
    ...
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        "<b>CDC Ingestion.</b> Source tables arrive via a Change Data Capture (CDC) "
        "connector, which adds system columns (<font face='Courier' size='8'>_is_deleted</font>, "
        "<font face='Courier' size='8'>_updated_at</font>) for change tracking.",
        styles['BodyText2']))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        ...
        "table is detected, the system autonomously creates a change event stream and a Bronze materialized view using "
        "<font face='Courier' size='8'>schema-on-read capture</font>, and registers the event in metadata.",
        styles['BodyText2']))
    elements.append(Spacer(1, 4))
    elements.append(Paragraph(
        "This schema-on-read pattern provides a critical advantage: the Bronze layer absorbs schema changes "
    ...
    add_callout(elements,
        ...
        "\"mechanical advantage, akin to moving the fulcrum on a lever closer to the load\" [6]. By enriching data with "
    ...
    trigger_rows = [
        ...
        ["Schema Drift", "Current Bronze payload keys are compared against the last-known schema snapshot. "
    ...
    elements.append(Spacer(1, 2))
    planner_rows = [
        ["Schema Context", "Bronze semi-structured keys, inferred types, sample values, cardinality estimates. For CUSTOMERS_RAW: "
         "CUSTOMER_ID is integer PK, EMAIL contains valid addresses, _UPDATED_AT is the CDC timestamp."],
        ...
        ["Knowledge Graph", "Vector similarity search finds analogous tables transformed before. If INVOICES_RAW arrives "
         "and ORDERS_RAW used a specific CDC dedup pattern, that pattern is surfaced. Also provides downstream dependency info."],
    ...
    elements.append(Paragraph(
        ...
        "dedup on _UPDATED_AT, extract 12 columns, derive RECENCY_DAYS, apply churn prediction directive by "
    ...
    elements.append(Paragraph(
        ...
        "type-safe column extraction from the semi-structured payload, derived columns from Directives, a target freshness interval, and soft-delete filtering.",
        styles['BodyText2']))
    elements.append(Spacer(1, 4))
    add_callout(elements,
        "<b>The Self-Correction Loop:</b> If the DDL fails compilation, the platform error message is injected back into "
        "the LLM prompt: <i>\"Your DDL failed with: 'invalid identifier SRC:full_name'. The payload contains "
    ...
    reflector_rows = [
        ...
        ["Failure Patterns", "\"Nested arrays require lateral expansion.\" \"LEAD()/LAG() on SUPPORT_TICKETS causes "
    ...
    elements.append(Paragraph(
        ...
        "engineer corrects an agent\u2019s join logic, that \"scar tissue\" is saved forever in the system\u2019s "
    ...
    elements.append(Paragraph(
        ...
        "derived, not primary. Because every Silver and Gold table is defined by a materialized view DDL (stored in logs), "
    ...
    elements.append(Paragraph(
        ...
        "rather than independently maintained data stores [7].",
        styles['BodyText2']))
...
def build_section4(elements, styles):
  ...
  {"name": "IS_DELETED",  "type": "BOOLEAN", "required": true}
    ...
    add_callout(elements,
        ...
        "micro-partition skew on CUSTOMER_ID. The platform\u2019s adaptive optimization cannot resolve the skew within the "
        "target freshness interval.\"<br/><br/>"
...
def build_section5(elements, styles):
    ...
    elements.append(Spacer(1, 4))
    bootstrap = [
        ["1. Structural\nDiscovery", "A catalog scanner reads scans every schema, table, and column. "
...
def build_section6(elements, styles):
    ...
    elements.append(Paragraph(
        ...
        "that CUSTOMERS_RAW should produce a typed CUSTOMERS Silver table, or that CUSTOMERS + ORDERS + "
...
def build_section7(elements, styles):
    ...
    elements.append(Paragraph(
        ...
        "engineer is less a builder of pipelines and more a designer of data systems\" [8]. Forbes' 2026 AI predictions "
        "similarly identify \"the shift from manual engineering to AI-orchestrated data workflows\" as a defining trend [9].",
        styles['BodyText2']))
    elements.append(Spacer(1, 10))
...
def build_section8(elements, styles):
    ...
    elements.append(HRFlowable(width="100%", thickness=1, color=SF_BLUE, spaceAfter=8))
    elements.append(Paragraph(
        "The Agentic Data Foundry is a deployable pattern. A reference implementation includes:",
        styles['BodyText2']))
    elements.append(Spacer(1, 4))

    components = [
        ["Bronze Materialized Views", "Schema-on-read materialized views", "5"],
        ["Silver Materialized Views", "LLM-generated, CDC-aware", "5"],
        ["Gold Materialized Views", "LLM-generated, multi-source aggregation", "4+"],
        ["Stored Procedures", "Python + SQL orchestration", "20+"],
        ...
        ["Management Interface", "Web-based management UI", "1"],
    ]
    ...
    elements.append(Paragraph(
        ...
        "landing via CDC, through Bronze onboarding, Silver transformation, Gold aggregation, Knowledge Graph "
        "enrichment, and semantic layer generation \u2014 with zero human-written transformation code.",
        styles['BodyText2']))
    elements.append(Spacer(1, 10))
...
def build_section9(elements, styles):
    ...
    elements.append(Paragraph(
        "The Bronze layer uses <font face='Courier' size='8'>schema-on-read capture</font> to preserve the complete source "
        "payload as a semi-structured column. This design ensures schema changes in the source never break Bronze, historical "
    ...
    elements.append(Paragraph("9.2 Declarative Materialization Over Imperative ETL", styles['SubsectionTitle']))
    elements.append(Paragraph(
        "Silver and Gold tables are implemented as declarative materialized views with a target freshness interval "
        "rather than stored procedure-based ETL. This provides declarative refresh semantics (the platform manages scheduling), "
        "automatic dependency tracking (downstream views refresh when upstream changes), and built-in observability "
    ...
    guardrails = [
        ...
        ["2. Semantic\nReference Check", "VALIDATE_GOLD_DDL parses SQL, extracts FROM/JOIN table references, verifies each exists in the system catalog. "
    ...
    elements.append(Paragraph(
        ...
        "Row access policies and dynamic data masking propagate through the materialized view dependency chain, "
...
def build_section10(elements, styles):
    ...
    elements.append(Paragraph(
        "The most frequent objection to agentic workflows is the cost of LLM inference. Critics point to the compute "
        "credits consumed by LLM services during the planning and validation phases.",
        styles['BodyText2']))
    elements.append(Spacer(1, 4))
    add_callout(elements,
    ...
    elements.append(Paragraph(
        ...
        "dollar invested</b> in generative AI projects, with top-performing organizations achieving up to 10.3\u00d7 [10]. "
        "Deloitte's 2026 State of AI survey found that <b>66% of enterprises report measurable productivity gains</b> "
        "from AI adoption, with an average 21% productivity improvement and 15% cost reduction [11]. NVIDIA's 2026 State "
    ...
    elements.append(Paragraph(
        ...
        "customer\u2019s platform metadata.",
        styles['BulletItem']))
    elements.append(Spacer(1, 2))
    elements.append(Paragraph(
        "\u2022 This creates a defensive moat that a generic, off-the-shelf LLM can never replicate. Your data platform "
    ...
    elements.append(Paragraph(
        ...
        "and available for human review in the management interface before a single table is touched. This is not a "
...
def build_section11(elements, styles):
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        "The trajectory is clear. Every major cloud data platform is investing in autonomous AI capabilities "
        "that enables business users to move \"from intent to actions and outcomes\" on governed data \u2014 "
        "demonstrates that the agentic paradigm is not a research concept but an emerging product category.",
        styles['BodyText2']))
    elements.append(Spacer(1, 6))

    snowwork = [
        ["Back-of-House\n(Foundry)", "Ingest \u2192 Transform \u2192 Validate \u2192 Materialize governed Gold tables + Semantic Layer", "Data Engineering\n(Agentic)"],
        ["Hand-off Point", "Semantic layer with dimensions, measures, synonyms, verified queries", "Shared Contract"],
        ["Front-of-House\n(Agentic AI)", "Natural language \u2192 SQL \u2192 Results \u2192 Actions \u2192 Outcomes", "Business Users\n(Agentic)"],
    ]
    ...
    elements.append(Paragraph(
        "The Foundry produces the governed semantic layer that front-of-house agentic platforms consume for business end-users. "
        "Without the Foundry, those platforms operate on manually curated Gold tables \u2014 recreating the same bottleneck "
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        "The Agentic Enterprise isn\u2019t a future state; with the right architecture, it\u2019s the current reality. "
...
def build_references(elements, styles):
    ...
    refs = [
        ...
        '[4] Databricks, "How AI Is Transforming Data Engineering," Databricks Blog, 2025. databricks.com',
        '[5] Capgemini, "From Data Pipelines to AI-Driven Integration," Capgemini Insights, 2025. capgemini.com',
        '[6] J. Heisler and G. Frere, "AI-Infused Pipelines," Builders Blog (Medium), Oct 2024. medium.com',
        '[7] InfoQ, "The End of the Bronze Age: Rethinking the Medallion Architecture," 2025. infoq.com',
        '[8] Data Engineering Academy, "The Future of Data Engineering in an AI-Driven World," 2025. dataengineeracademy.com',
        '[9] M. Minevich, "Agentic AI Takes Over \u2014 11 Shocking 2026 Predictions," Forbes, Dec 2025. forbes.com',
        '[10] IDC, "Business Opportunity of AI: GenAI Delivering New Business Value and Increasing ROI," 2025. idc.com',
        '[11] Deloitte, "The State of AI in the Enterprise," Deloitte AI Institute, 2026. deloitte.com',
        '[12] NVIDIA, "State of AI Report 2026," Mar 2026. blogs.nvidia.com',
        '',
        '',
    ]
    for ref in refs:
    ...
    elements.append(Paragraph(
        ...
        'github.com/dbaontap/agentic-data-foundry. A reference implementation built on Snowflake is available '
        'at the repository.</i>',
        styles['SmallNote']))
